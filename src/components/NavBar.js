...
const NavBar = () => {
    ...
    return (
        <nav className="bg-white shadow-md p-4 font-[Poppins] text-[14px] font-semibold relative">
                ...
                <ul className="hidden lg:flex items-center gap-8 text-black">
                    {[
                        ...
                        <li key={item.path}>
                            <Link 
                                href={item.path} 
                                className={`${pathName === item.path ? "text-green-700 border-b-2 border-green-700" : "hover:text-green-700 hover:border-b-2 hover:border-green-700"} pb-1`}
                            >
                                {item.name}
                            </Link>
                        </li>
            ...
            <div className={`fixed top-0 right-0 w-3/4 md:w-1/2 h-full bg-green-700 text-white shadow-lg z-50 transition-transform duration-300 ease-in-out ${
                ...
                <ul className="flex flex-col items-center mt-6 gap-1"> {/* Tighter spacing */}
                    {[
                        { name: "Home", path: "/" },
                        { name: "About Us", path: "/about" },
                        { name: "Products", path: "/products" },
                        ...
                        <li 
                            key={item.path} 
                            className={`w-full text-center py-1 ${ // Less padding
                                pathName === item.path ? "bg-green-800 text-white" : "hover:bg-green-600"
                            }`}
                            onClick={() => setIsMenuOpen(false)} // Close on click
                        >
                            <Link href={item.path} className="block w-full px-2 py-1"> {/* Reduced padding */}
                                {item.name}
                            </Link>
                        </li>
                    ))}
